...
class AcademicLevel():
    ...
    def getCoauNumLevel(self):
        ...
        for author in self.authors:
            ...
            coauNum = len(coaus)
            coauAvgLevel = sum([float(self.authorsPN.get(coau)) for coau in coaus]) / coauNum
            authorLevel = float(self.authorsPN.get(author))
    ...
    def saveCoauNumLevel(self):
        with open(OUTPUT_COAUNUM_LEVEL_PAPERNUM, 'w') as fileWriter:
